...
interface EditSupplierModalProps {
  isOpen: boolean;
  onClose: () => void;
  supplierData: Fornecedor;
  onSave: (supplier: Fornecedor) => void;
}

const EditSupplierModal: React.FC<EditSupplierModalProps> = ({ isOpen, onClose, supplierData, onSave }) => {
  const [formData, setFormData] = useState<Fornecedor>(supplierData);
  const [errors, setErrors] = useState<{ [key: string]: string }>({});

  useEffect(() => {
    // When the modal is opened with new data, update the form
  ...
  return (
    <div 
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="editSupplierModalTitle"
    >
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[95vh] flex flex-col">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 id="editSupplierModalTitle" className="text-xl font-bold text-slate-800">Editar Fornecedor</h2>
